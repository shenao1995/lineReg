...
# %% auto 0
__all__ = ['MaskNormalizedCrossCorrelation2d', 'MaskGradientNormalizedCrossCorrelation2d']

# %% ../notebooks/api/05_metrics.ipynb 4
class MaskNormalizedCrossCorrelation2d(torch.nn.Module):
...
class MaskGradientNormalizedCrossCorrelation2d(MaskNormalizedCrossCorrelation2d):
...
class Sobel(torch.nn.Module):
    def __init__(self, sigma):
        ...
        Gx = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]]).to(torch.float32).to('cuda')
        Gy = torch.tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]).to(torch.float32).to('cuda')
